...
function AIInsights({ shipment }) {
  ...
  const callGeminiAPI = async () => {
    ...
    const prompt = `
You are a pharmaceutical cold-chain risk analyst. Analyze the following shipment temperature data and provide a structured assessment.

Shipment ID: ${shipment.id}
Location: ${shipment.location}
Current Temperature: ${shipment.temperature}°C
Temperature History (last 5 readings): ${shipment.history.join(', ')}°C

Safe range: ≤ 8°C | Warning: 8–10°C | Critical: > 10°C

Respond in EXACTLY this JSON format with no markdown or extra text:
{
  "riskLevel": "Low" or "Medium" or "High",
  "explanation": "2-3 sentence analysis of the temperature trend and what it means for drug integrity",
  "suggestedAction": "Specific actionable recommendation for the logistics team"
}
`
    ...
    try {
      ...
      setError(err.message || 'Failed to get AI insights. Check your API key.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="mt-4">
      ...
      <button
        ...
        disabled={loading}
        className={`w-full flex items-center justify-center gap-3 py-3 px-6 rounded-xl text-sm font-medium transition-all duration-300 ${
          loading ? 'bg-slate-700/50 cursor-not-allowed' : 'bg-blue-500/10 hover:bg-blue-500/20 border border-blue-400/30 hover:border-blue-400/50 hover:scale-105'
        }`}
        style={{
